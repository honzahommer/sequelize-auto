...
AutoSequelize.prototype.build = function(callback) {
  ...
  function mapTable(table, _callback){
    self.queryInterface.describeTable(table, self.options.schema).then(function(fields) {
      ...
      if (self.options.dialect !== 'mysql') {
        _callback();
      } else {
        self.queryInterface.sequelize.query(`SHOW FULL COLUMNS FROM ${table}`, {
          type: self.sequelize.QueryTypes.SELECT, raw: true
        }).then(function(columns) {
          columns.forEach(({ Comment, Field }) => {
            if (Comment) {
              self.tables[table][Field].comment = Comment;
            }
          });
...
AutoSequelize.prototype.run = function(callback) {
  ...
  function generateText(err) {
    ...
    async.each(_.keys(self.tables), function(table, _callback){
      ...
      var tableName = self.options.camelCase ? _.camelCase(table) : (self.options.snakeCase ? _.snakeCase(table) : table);
      ...
      _.each(fields, function(field, i){
        ...
        var fieldName = self.options.camelCase ? _.camelCase(field) : (self.options.snakeCase ? _.snakeCase(field) : field);
        text[table] += spaces + spaces + (/^\w+$/.test(fieldName) ? fieldName : `"${fieldName}"`) + ": {\n";
          ...
          if (attr === "foreignKey") {
            if (isSerialKey) {
              ...
              if(self.options.schema) {
                text[table] += ",\n" + spaces + spaces + spaces + spaces + spaces + "schema: \'" + self.tables[table][field][attr].foreignSources.target_schema + "\'"
              }
        ...
        if (self.options.camelCase || self.options.snakeCase) {
        ...
        if(self.options.typescript) tsTableDef += tsHelper.def.getMemberDefinition(spaces, fieldName, tsVal, tsAllowNull, tsAutoIncrement, tsPrimaryKey);
      });
      ...
      if(self.options.typescript) typescriptFiles[1] = tsHelper.model.generateTableModels(_.keys(text), self.options.spaces, self.options.indentation, self.options.camelCase, self.options.camelCaseForFileName, self.options.snakeCase, self.options.snakeCaseForFileName);
...
AutoSequelize.prototype.write = function(attributes, typescriptFiles, callback) {
  ...
  function createFile(table, _callback) {
    var fileName = self.options.camelCaseForFileName ? _.camelCase(table) : (self.options.snakeCaseForFileName ? _.snakeCase(table) : table);
