...
function generateTableModels(tableNames, isSpaces, indentation, isCamelCase, isCamelCaseForFile, isSnakeCase, isSnakeCaseForFile) {
  var spaces = '';
  ...
  function generateInterface() {
    ...
    for (var i = 0; i < tableNames.length; i++) {
      var table = isCamelCase ? _.camelCase(tableNames[i]) : (isSnakeCase ? _.snakeCase(tableNames[i]) : tableNames[i]);
  ...
  function generateTableMapper() {
    ...
    for (var i = 0; i < tableNames.length; i++) {
      var tableForClass = isCamelCase ? _.camelCase(tableNames[i]) : (isSnakeCase ? _.snakeCase(tableNames[i]) : tableNames[i]);
      var tableForFile = isCamelCaseForFile ? _.camelCase(tableNames[i]) : isSnakeCaseForFile ? _.snakeCase(tableNames[i]) : tableNames[i];
...
function getMemberDefinition(spaces, fieldName, val, allowNull, autoIncrement, primaryKey) {
  if (fieldName === undefined) return '';
  var isRequiredField = (allowNull === true || primaryKey === true) && autoIncrement === true;
